...
/*----------------------------------------------------------------
    ...
    Modification Description：Use Post method to get string result, modify form processing method
    ...
    Modification Identifier：Senparc - 20190521
    Modification Description：v0.8.4 Cancel UrlEncode encoding for fileName parameter in HttpUtility.HttpPost_Common_NetCore's CreateFileContent
...
namespace Senparc.CO2NET.HttpUtility
{
    ...
    public static partial class RequestUtility
    {
        ...
        private static void HttpClientHeader(HttpClient client, string refererUrl, bool useAjax, Dictionary<string, string> headerAddition = null, int timeOut = Config.TIME_OUT)
        {
            ...
            var userAgentSetted = false;//Whether User-Agent is already customized
            if (headerAddition != null)
            {
                foreach (var item in headerAddition)
                {
