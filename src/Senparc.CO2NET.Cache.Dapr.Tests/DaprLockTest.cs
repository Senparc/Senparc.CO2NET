namespace Senparc.CO2NET.Cache.Dapr.Tests
{
    ...
    public class DaprLockTest
    {
        ...
        {
            var cache = DaprTestConfig.GetCacheStrategy();
            ...
            var startRun = false;
            // Configure all threads (to start as simultaneously as possible, ignore setting time, so set first, then start quickly in sequence)
