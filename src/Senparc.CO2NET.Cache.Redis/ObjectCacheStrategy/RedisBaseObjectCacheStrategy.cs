...
namespace Senparc.CO2NET.Cache.Redis
{
    ...
    /// 所有Redis基础缓存策略的基类
    /// </summary>
    public abstract class RedisBaseObjectCacheStrategy : BaseCacheStrategy, IBaseObjectCacheStrategy
    {

        public ConnectionMultiplexer Client { get; set; }
        protected IDatabase _cache;

        protected RedisBaseObjectCacheStrategy()
        {
            Client = RedisManager.Manager;
            _cache = Client.GetDatabase();
        }

        static RedisBaseObjectCacheStrategy()
        {
            //全局初始化一次，测试结果为319ms
